...
public class TransactionEvent extends Event {
    ...
    public TransactionEvent(LambdaSpan span) {
        ...
        if (span.context() != null && span.context() instanceof LambdaSpanContext) {
            ...
            final DistributedTracing dt = DistributedTracing.INSTANCE;
            final DistributedTracingState state = context.getDistributedTracingState();
            intrinsics.putAll(dt.getDistributedTracingAttributes(state, context.getTransactionState().getTransactionId(), span.priority()));
